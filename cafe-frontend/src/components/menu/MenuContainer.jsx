...
const MenuContainer = () => {
  ...
  return (
    <>
      <div className="grid grid-cols-4 gap-4 px-10 py-6 w-[100%]">
        {menus.map((menu) => {
          ...
          return (
            <div
              key={menu.id}
              className={`
                relative flex flex-col items-start justify-between p-6 rounded-2xl h-[120px] cursor-pointer 
                transition-all duration-300 transform hover:scale-105 hover:shadow-retro
                ${isSelected 
                  ? 'bg-gradient-retro border-2 border-retro-orange shadow-retro' 
                  : 'bg-gradient-card border border-dark-700 hover:border-retro-orange'
                }
              `}
              onClick={() => {
                setSelected(menu);
